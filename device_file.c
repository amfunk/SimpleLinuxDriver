...
int register_device(void)
{
  ...
  {
    printk( KERN_WARNING "Simple-driver: can't create device class with errorcode = %li\n", PTR_ERR( device_class) );
    unregister_chrdev( device_file_major_number, device_name );
    return PTR_ERR( device_class );
  }
  printk( KERN_NOTICE "Simple-driver: Device class created\n" );
  ...
  {
    printk( KERN_WARNING "can't create device with errorcode = %li\n", PTR_ERR( device_struct ) );
    class_destroy( device_class );
    unregister_chrdev( device_file_major_number, device_name );
    return PTR_ERR( device_struct );
  }
  printk( KERN_NOTICE "Simple-driver: Device created\n" );
...
void unregister_device(void)
{
  ...
  if ( !IS_ERR( device_class ) )
  {
    ...
    printk( KERN_NOTICE "Simple-driver: Device destroyed\n" );
  }
  if ( !IS_ERR( device_class ) && !IS_ERR( device_struct ) )
  {
    class_destroy( device_class );
    printk( KERN_NOTICE "Simple-driver: Device class destroyed\n" );
  }
  if(device_file_major_number != 0)
  {
    unregister_chrdev(device_file_major_number, device_name);
    printk( KERN_NOTICE "Simple-driver: Device unregistered\n" );
  }
}
